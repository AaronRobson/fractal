...
class GUI(tk.Frame):
	...
	def Bindings(self):
		#Set keybindings corresponding to the accelerator text of the menu items, requires the event placeholder in each of the methods called
		#file
		...
		#if the user keys to quit then holding shift while releasing it will nullify the close of the program
	...
	def CreateMenu(self):
		...
		self.casmenu.file.add_command(label='Reset', command=self.Reset, accelerator='Ctrl+R')
		self.casmenu.file.add_command(label='Open', command=self.OpenFile, accelerator='Ctrl+O')
		self.casmenu.file.add_command(label='Save As', command=self.SaveFile, accelerator='Ctrl+S')
		self.casmenu.file.add_separator()
		self.casmenu.file.add_command(label='Exit', command=self.Quit, accelerator='Esc')

		#define Edit branch
		self.casmenu.edit = tk.Menu(self.casmenu, tearoff=self.menuTearOff)
		self.casmenu.edit.add_command(label='Undo', command=self.Undo, accelerator='Ctrl+Z')
		self.casmenu.edit.add_separator()
		self.casmenu.edit.add_command(label='Cut', command=self.Cut, accelerator='Ctrl+X')
		self.casmenu.edit.add_command(label='Copy', command=self.Copy, accelerator='Ctrl+C')
		self.casmenu.edit.add_command(label='Paste', command=self.Paste, accelerator='Ctrl+V')

		#define Help branch
		self.casmenu.help = tk.Menu(self.casmenu, tearoff=self.menuTearOff)
		self.casmenu.help.add_command(label='Help', command=self.Help, accelerator='F1')
		self.casmenu.help.add_separator()
		self.casmenu.help.add_command(label='About', command=self.About, accelerator='F12')
	...
	def CreateControls(self):
		...
		#column span must be one more than number of previous controls (to allow the blank extra one to take precidence in absorbing extra space)

		self.cnv_Image = tk.Canvas(bd=0, width=self.sideLength, height=self.sideLength, background=self.backgroundCol)
	...
	def SaveFile(self, event=None):
		filename = tkFileDialog.asksaveasfilename(filetypes=self.fileFormats, defaultextension='')
		...
		if len(filename) > 0:
			...
			bufferData = ('\t<polygon points="%s,%s %s,%s %s,%s"' % self.Flatten(tri) + fill for tri in self.st.triangles)
			...
			#print(buffer) #can be slow on large files
			#Write to file
			inFile.write('\n'.join(buffer))
	...
	def Quit(self, event=None):
		#if no changes have been made since loaded or created
		#	make confirm box with "are you sure you want to close" title, quit button, cancel button (with focus)
		#else:
		#	make confirm box with "are you sure you want to close" title, quit button, save button, cancel button (with focus)
		self.master.destroy()
	...
	def Display(self):
		...
		self.vValuesStored.set('Values Stored = %d' % (self.st.NumOfTriangles()*3*2))
		...
		[self.cnv_Image.create_polygon(fill=self.foregroundCol, *triangle) for triangle in self.st.triangles]
